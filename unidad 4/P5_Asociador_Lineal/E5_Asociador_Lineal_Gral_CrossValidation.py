...
#3
def verificar_balance(Y_subconjunto, nombre_subconjunto):
    total = Y_subconjunto.shape[1]
    proporciones = [0, 0, 0]  # Buena, Regular, Malo

    for columna in Y_subconjunto.T:
        index_max = list(columna).index(max(columna))
        proporciones[index_max] += 1

    # Calculamos la diferencia entre la clase más frecuente y la menos frecuente
    max_clase = max(proporciones)
    min_clase = min(proporciones)

    if max_clase == min_clase:
        print(f"Las clases en {nombre_subconjunto} están balanceadas.")
    else:
        print(f"Las clases en {nombre_subconjunto} no están balanceadas.")
        print(f"Proporción de clases en {nombre_subconjunto}:")
        for i, clase in enumerate(Clases):
            porcentaje = (proporciones[i] / total) * 100 if total > 0 else 0
            print(f"  {clase}: {porcentaje:.2f}%")

# Define la función ANTES de llamarla
def cross_validation_kfold(X, Y, K=5):
    total_casos = X.shape[1]
    indices = list(range(total_casos))
    import random
    random.shuffle(indices)
    fold_size = total_casos // K
    eficiencias = []
    ...
    for fold in range(K):
        ...
        for i in range(X_val.shape[1]):
            ...
            if IndexMaxYcasoi == IndexMaxYrealcasoi:
                casosCorrectos += 1
        eficiencia = casosCorrectos / X_val.shape[1] * 100.0
        eficiencias.append(eficiencia)
        print(f"Fold {fold+1}: Eficiencia = {eficiencia:.2f}%")

    print(f"\nEficiencia promedio en {K}-Fold Cross Validation: {sum(eficiencias)/K:.2f}%")
...
#3 Llamadas
print(" ")
verificar_balance(Y_train, "entrenamiento")
verificar_balance(Y_val, "validación")

print("\n========== INICIO CROSS VALIDATION ==========")
cross_validation_kfold(X, Y, K=5)
